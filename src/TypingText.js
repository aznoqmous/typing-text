export default class TypingText {
    ...
    async writeText(text, element){
        ...
        return new Promise(res => {
            const loop = ()=>{
                ...
                currentIndex++
                if(currentIndex < text.length) setTimeout(()=> loop(), Math.random() * (this.opts.minSpeed - this.opts.maxSpeed)+this.opts.maxSpeed)
                else res()
            }
            loop()
        })
    ...
    static async write(element, html, opts={}){
        return await (new TypingText(element, html, opts)).do()
